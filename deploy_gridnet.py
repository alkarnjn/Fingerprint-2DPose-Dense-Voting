...
def deploy(args, model, model_params, prefix):
    ...
    pose_file = osp.join(prefix + "_feature", "pose_2d", model_params.exp_name, args.img_name.split(".")[0] + ".txt")
    show_file = osp.join(prefix + "_feature", "pose_2d", model_params.exp_name, args.img_name.split(".")[0] + ".png")
    ...
    pose_2d[0] = pose_2d[0] / scale
    pose_2d[1] = pose_2d[1] / scale
    pose_2d[2] = (-pose_2d[2] + 180) % 360 - 180

    uni_io.mkdir(osp.dirname(pose_file))
    np.savetxt(pose_file, [pose_2d], fmt="%.2f")
